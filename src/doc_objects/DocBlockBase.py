...
class DocBlockBase(object):
    ...
    @property
    def attributes(self):
        return self.__attr_values.values()
    ...
    def _link_definitions(self):
        for link_def in self.__link_defs.values():
            yield link_def
    ...
    def _def_attribute(self, name, help_txt, expected=False, markdown=False,
        ...
        if self.__attr_defs.has_key(name):
    ...
    def _def_link(self, name, attr_name, links_to, multiple=False):
        ...
        '''
        if self.__link_defs.has_key(name):
    ...
    def _def_trans_property(self, name, property_chain):
        ...
        if self.__link_defs.has_key(name):
            ...
            raise IndexError(msg % (name))
        if self.__trans_properties.has_key(name):
    ...
    def _def_reverse_link(self, name, from_doc_class, from_link_name):
        ...
        if not self.__link_defs.has_key(name):
    ...
    def get_embedded_blocks(self):
        ...
        for name in self.__embedded_blocks_attrs:
            if self.__attr_values.has_key(name):
    ...
    def clear_links(self):
        ...
        self.__linked_objs = dict()
        for link in self.__link_defs.values():
...
#                             ", ".join(link.doc_classes)))
        ...
        for attr_def in self.__attr_defs.values():
            if attr_def.expected:
                if not self.__attr_values.has_key(attr_def.name):
    ...
    def _take_attributes_from(self, raw_doc_block):
        ...
        for attribute in raw_doc_block.attributes:
            if self.__attr_values.has_key(attribute.name):
            ...
            # Normal Attributes
            if self.__attr_defs.has_key(attribute.name):
                ...
                if emsg is None:
                    ...
                    try:
                        attribute.value = self._validate_value_class(
                            attribute.name, attribute.value)
                    except InvalidAttributeValueClass, e:
                        emsg = str(e)
    ...
    def __resolve_transitive_property(self, property_chain):
        if len(property_chain) == 1:
            ...
            if self.__link_defs.has_key(next_prop_name):
    ...
    def _validate_value_class(self, field_name, value):
        ...
        if self.__attr_defs.has_key(field_name):
    ...
    def all_todo_items_in_col(self):
        ...
        if not self.col.cache.has_key('todo'):
    ...
    def print_doc_class_help(self):
        ...
        if len(self.__trans_properties) > 0:
            for prop in self.__trans_properties.values():
                msg = "    %-20s - %s"
        ...
        # Links
        links = [l for l in self.__link_defs.values() if not l.is_reverse]
        if len(links) > 0:
            print "  Links:"
        ...
        # Reversed Links
        links = [l for l in self.__link_defs.values() if l.is_reverse]
        if len(links) > 0:
